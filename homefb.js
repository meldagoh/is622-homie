...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        window.addEventListener("DOMContentLoaded", () => getAllDataOnce(main_user));
        // ...
    } else {
        // User is signed out
        // ...
...
function getAllDataOnce(main_user) {
    ...
    get(child(dbRef, "property")).then((snapshot) => {
        ...
        for (var i = 0; i < property.length; i++) {
            ...
            const pTitle = property[i]?.title || "Untitled listing"
            const pAdd = property[i]?.address || "Address not provided"
            const pRent = property[i]?.financial?.rent ?? "N/A"
            const pBedroom = property[i]?.bedroomquantity ?? "N/A"
            const pBathroom = property[i]?.bathroomquantity ?? "N/A"
            const listId = property[i]?.listId || `property-${i}`
